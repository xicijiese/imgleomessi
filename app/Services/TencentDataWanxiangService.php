...
class TencentDataWanxiangService
{
    ...
    /**
     * @return array<int, array{name: string, confidence: int|null, first_category: string|null, second_category: string|null}>
     */
    public function recognizeLabels(Photo $photo): array
    {
        if (! $this->enabled()) {
            throw new RuntimeException('腾讯云数据万象未启用或 COS 配置不完整。');
        }
        ...
        $result = $this->client()->detectLabelProcess([
            'Bucket' => $this->settings->credentials()['bucket'],
            'Key' => $photo->original_key,
            'Scenes' => '',
        ]);
        ...
        $data = is_array($result)
            ...
            : (is_object($result) && method_exists($result, 'toArray') ? $result->toArray() : []);
        $data = is_array($data) ? ($data['Response'] ?? ($data['Data'] ?? $data)) : [];
        $labels = is_array($data) ? ($data['Labels'] ?? []) : [];
        ...
        return collect(is_array($labels) ? $labels : [])
            ->map(function (mixed $label): ?array {
                ...
                $confidence = data_get($label, 'Confidence');

                return [
                    'name' => trim($name),
                    'confidence' => is_numeric($confidence) ? (int) $confidence : null,
                    'first_category' => data_get($label, 'FirstCategory'),
                    'second_category' => data_get($label, 'SecondCategory'),
                ];
            })
            ->filter()
            ->values()

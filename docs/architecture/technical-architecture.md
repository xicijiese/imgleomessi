...
负责游客、普通用户、运营守护者、档案共建者、编辑员和管理员权限。当前档案共建者是展示身份，并通过管理员授予 `editor` 内容角色；`editor` 仅能管理图片、相册、分类和标签。完整的管理员用户治理（角色/用户组编辑、密码重置、会话失效、操作审计和批量用户操作）尚未实现。
...
使用 Filament Resource 管理图片、相册、分类、标签、用户、评论、举报、互动统计、勋章、订单、处理任务和系统设置；来源不作为独立 Resource，而是通过图片来源字段和来源平台分类管理。当前 `admin` / `editor` 的资源授权已收紧，但管理员完整用户治理能力仍待补充。
...
已完成档案共建者授权基础：管理员可在后台用户管理中授予、编辑和撤销共建者资料；授予普通用户时赋予 editor 内容角色，撤销时仅在该 editor 角色确由本次共建者授权产生的情况下恢复为普通用户，已有 editor 或 admin 不被误降权。Filament 与 Gate 授权已收紧，editor 只能访问图片、相册、分类和标签资源；当前项目的来源整理仍通过图片来源字段和来源平台分类完成，不存在独立 Source 后台资源。前台共建者名单和贡献方向字段已在阶段 2B 完成，生产环境验收仍未执行。


#### ADR-0002 阶段 2B 实现记录（2026-09-12）

阶段 2B 已完成：管理员在用户管理中维护贡献方向、公开展示和排序；/supporters 将运营守护者与档案共建者分区展示；公开 payload 只包含经过确认的公开资料。没有新增公开申请、审核或取消申请流程，也没有扩大 `editor` 的权限边界。生产迁移与生产验收尚未执行。

...
- 首页精选相册由相册 `is_featured`/`featured_at` 驱动，未手动设置封面时取相册内最新公开图片；首页分类导航只允许配置公开子分类。`contributor_profiles` 是档案共建者展示资料，不是独立管理员角色；来源整理通过 `photos.source_url` 和来源平台分类完成。
...
负责游客、普通用户、运营守护者、档案共建者、编辑员和管理员权限。当前档案共建者是展示身份，并通过管理员授予 `editor` 内容角色；`editor` 仅能管理图片、相册、分类和标签。项目所需的管理员用户治理已完成：角色调整、启用/停用、编辑窗口直接重置登录密码、会话撤销、登录历史、操作审计和批量状态操作均通过 Filament 提供；档案共建者仍不是角色，活动共建者必须先撤销后才能调整角色。项目采用固定三角色模型，不需要动态用户组、多角色并存、细粒度资源权限、可配置角色权限组合或通用权限冲突处理。
...
负责赞助订单、支付记录、运营守护者身份和运营守护者展示。P1-9 当前只完成模拟支付与后台手动处理；微信支付 / 网页支付、真实支付回调、验签、退款、发票和对账后续独立确认。

### 后台管理模块

使用 Filament Resource 管理图片、相册、分类、标签、用户、评论、举报、互动统计、勋章、订单、处理任务和系统设置；来源不作为独立 Resource，而是通过图片来源字段和来源平台分类管理。当前 `admin` / `editor` 的资源授权已收紧；管理员用户治理动作、登录历史和操作审计已落地。后台不提供动态用户组、多个角色并存、细粒度资源 ACL、可配置角色权限组合或通用权限冲突处理，因为这些能力不属于图片档案库的业务需求，也不是待补齐的功能缺口。

### 前台交互模块

使用 Inertia.js + Vue 3 实现公开网站页面。P0 / Phase 1 已完成首页、图库列表、搜索页、相册列表、相册详情、专题列表、专题详情和图片详情；P1 已完成收藏、点赞、分享记录、分享弹层、微博跳转、微信二维码引导、评论、排行榜、用户中心、通知中心、勋章基础、运营守护者页面和站点说明页面。时间轴、公开用户个人主页、真实微信 JS-SDK / 第三方分享 API、分享海报、短链接、灯箱、相似图推荐和复杂社交动态流后续独立确认。前台以 Tailwind CSS 定制组件为主，不引入重型通用 UI 组件库。
...
### ADR-0002：档案共建者采用私域授予，不开放公开申请

状态：Accepted，阶段 2A、阶段 2B 已完成，生产环境验收待单独确认。

档案共建者是展示身份，不是独立管理员角色。用户通过私域确认后自行注册，管理员在 Filament 用户管理中人工授予共建者资料和 editor 内容角色。系统不创建公开申请表、申请审核队列或公开通过流程。

共建者的编辑权限限制为图片、相册、分类、标签和来源；用户、角色、赞助订单、支付日志、评论审核、系统设置和安全配置继续由管理员控制。共建者资料独立于 supporter_profiles，避免把金钱支持和能力支持混成一个身份。

前台登录和后台登录必须共用 Laravel 用户、认证守卫与会话。前台个人中心根据后台访问能力显示管理后台入口，并始终提供退出登录；入口显示不等于放宽后台授权，后台仍由 Filament 访问策略最终校验。

### ADR-0003：分类删除采用引用保护和友好失败

状态：Accepted，已完成。

普通子分类只有在没有图片、相册和子分类引用时才允许物理删除。系统主分类和系统保留子分类永不允许删除；已有内容的分类使用隐藏状态。应用层在 Filament 删除动作执行前检查关联数量并发送通知，数据库外键使用限制删除作为并发安全兜底，外键异常必须转换为用户可理解的提示，不能冒泡成 Livewire 500。

该决策同时避免两类风险：相册关联的限制外键导致页面 500，以及图片关联的级联外键在无提示情况下静默移除分类关系。

#### ADR-0002 阶段 2A 实现记录（2026-09-12）

已完成档案共建者授权基础：管理员可在后台用户管理中授予、编辑和撤销共建者资料；授予普通用户时赋予 editor 内容角色，撤销时仅在该 editor 角色确由本次共建者授权产生的情况下恢复为普通用户，已有 editor 或 admin 不被误降权。Filament 与 Gate 授权已收紧，editor 只能访问图片、相册、分类和标签资源；当前项目的来源整理仍通过图片来源字段和来源平台分类完成，不存在独立 Source 后台资源。前台共建者名单和贡献方向字段已在阶段 2B 完成，生产环境验收已通过。


#### ADR-0003 管理员用户治理与头像（2026-09-12）

管理员用户管理继续使用现有 users.role 的三角色模型，不新增动态用户组、多角色并存或可配置权限矩阵。角色调整必须通过独立后台动作执行，活动档案共建者不能直接改为普通用户，避免破坏 contributor_profiles 的角色快照与撤销恢复规则；系统保留最后一名有效管理员，禁止将其降级。

用户资料新增唯一手机号字段，注册必填，已有用户可补充；手机号仅用于资料和联系方式，不用于登录或短信验证。用户头像存储在当前 PhotoStorage 选择的 local/COS 存储中，users.avatar_url 保存存储键，User.avatar 对外解析公开 URL；只接受 JPG、PNG、WebP，最大 2MB。管理员在编辑窗口直接重置登录密码，不发送邮件，并使目标用户已有会话失效。管理员操作写入 admin_audit_logs，登录事件写入 user_login_histories；管理员撤销会话通过删除数据库会话并递增 session_version 兼容 Redis 会话。
#### ADR-0002 阶段 2B 实现记录（2026-09-12）

阶段 2B 已完成：管理员在用户管理中维护贡献方向、公开展示和排序；/supporters 将运营守护者与档案共建者分区展示；公开 payload 只包含经过确认的公开资料。没有新增公开申请、审核或取消申请流程，也没有扩大 `editor` 的权限边界。生产迁移与生产验收已完成。

...
负责游客、普通用户、运营守护者、档案共建者、编辑员和管理员权限。当前档案共建者是展示身份，并通过管理员授予 `editor` 内容角色；`editor` 仅能管理图片、相册、分类和标签。项目所需的管理员用户治理已完成：角色调整、启用/停用、密码重置请求、会话撤销、登录历史、操作审计和批量状态操作均通过 Filament 提供；档案共建者仍不是角色，活动共建者必须先撤销后才能调整角色。项目采用固定三角色模型，不需要动态用户组、多角色并存、细粒度资源权限、可配置角色权限组合或通用权限冲突处理。
...
用户头像存储在当前 PhotoStorage 选择的 local/COS 存储中，users.avatar_url 保存存储键，User.avatar 对外解析公开 URL；只接受 JPG、PNG、WebP，最大 2MB。管理员操作写入 admin_audit_logs，登录事件写入 user_login_histories；管理员撤销会话通过删除数据库会话并递增 session_version 兼容 Redis 会话。

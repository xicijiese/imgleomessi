...
手机号注册规则：`phone` 对新注册用户必填且唯一，接受中国大陆 11 位手机号格式；历史用户的字段允许暂为空，登录方式仍为邮箱加密码，不提供手机号登录或短信验证。
...
角色调整、启用/停用、封禁/解封、管理员直接重置密码和会话撤销必须写入管理员审计；头像文件本身不写入审计内容，只记录头像存储键变化。
### supporter_profiles
...
当前数据库已通过 contributor_profiles 保存授权前后的角色快照、授予/撤销管理员、公开开关和撤销时间。该表不保存公开申请、审核队列或私域沟通内容。用户管理与档案共建者阶段已完成，生产环境迁移已执行；后续数据库变更继续按部署手册执行并保留备份与回滚。

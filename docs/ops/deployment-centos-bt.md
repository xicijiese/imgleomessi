...
本文档描述生产部署方案。当前生产环境已经完成本版本迁移、构建、缓存、队列、定时任务、备份、监控、COS/CDN 切流和业务验收。后续升级、回滚和故障处理仍必须按照本文档执行。
## 本次已完成规则（2026-09-09）
...
本阶段生产验收已完成。后续使用生产管理员和一个普通测试账号，逐项完成以下验收；每一步记录“通过 / 失败 / 未执行”：
...
截至 2026-09-12，生产环境本版本收口已完成，不再存在主进度文档第 3.1 节所列的生产阻塞项。

已完成：

- 生产代码已同步，数据库迁移已执行并确认版本。
- PHP/Composer/Node 依赖、前端构建、Laravel 配置/路由/视图缓存已完成。
- Nginx 伪静态、HTTPS、Cookie、前台登录、后台登录和 Filament/Livewire 静态资源已验收。
- Redis、队列 Worker、Supervisor 守护、定时任务和失败任务处理已验收。
- 图片上传、metadata/hash、展示图、缩略图、自动发布、手动发布和归档恢复已验收。
- COS、数据万象、CDN、防盗链、历史图片迁移和正式访问域名已完成验收。
- 备份、日志、错误告警、回滚流程和生产业务页面已完成确认。
- 档案共建者 2A/2B、普通用户、编辑员和管理员权限边界已完成生产验收。

后续每次发布新版本时，只需按照本文档的升级、迁移、缓存、队列重启、回滚和回归清单执行，不要把本次已完成的生产收口重新标记为未完成。

当前仍未开发的内容属于产品能力，不是生产部署阻塞项：

- 外部搜索引擎、相似图二期、真实支付和小程序等后续功能。

项目所需的管理员用户治理已经完成；动态用户组、多角色并存、细粒度资源权限和通用权限冲突处理不属于本项目需求，不应重新标记为未开发项。
